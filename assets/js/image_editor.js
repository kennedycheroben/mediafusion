...
(function () {
    ...
    function createAdvancedControls() {
        ...
        // Attach event listeners for dynamically created controls
    ...
    function fitToView() {
        ...
        // Validate dimensions
        if (vw <= 0 || vh <= 0 || imgNatW <= 0 || imgNatH <= 0) return;
        if (!isFinite(vw) || !isFinite(vh) || !isFinite(imgNatW) || !isFinite(imgNatH)) return;
        ...
        try {
            ...
            if (!isFinite(zoom) || !isFinite(panX) || !isFinite(panY)) {
                zoom = 1;
                panX = 0;
                panY = 0;
            }
            ...
            console.error("fitToView error:", error);
        }
    }
    ...
    function applyFiltersToCanvas(context) {
        if (!sourceImg || !context) return;
        ...
        try {
            context.filter = filterChain.length > 0 ? filterChain.join(' ') : 'none';
        } catch (e) {
    ...
    function render() {
        ...
        try {
            ...
            // Validate dimensions
            if (vw <= 0 || vh <= 0 || !isFinite(vw) || !isFinite(vh)) return;
            
            // Reset canvas dimensions (this clears the context)
            mainCanvas.width = vw;
            ...
            // Use resetTransform if available, otherwise fallback to setTransform
            try {
                ...
                // ignore if context is already in a bad state
            }
            ctx.save();
            
            // Validate drawing parameters
            if (!isFinite(panX) || !isFinite(panY) || !isFinite(zoom)) {
                ctx.restore();
                return;
            }
            
            const displayWidth = imgNatW * zoom;
            const displayHeight = imgNatH * zoom;
            ...
            const centerX = panX + displayWidth / 2;
            const centerY = panY + displayHeight / 2;
            ...
            ctx.globalAlpha = Math.min(1, Math.max(0, adjustments.opacity / 100));
            // applyFiltersToCanvas expects a 2D context
            applyFiltersToCanvas(ctx);

            // Draw image
            ctx.drawImage(sourceImg, panX, panY, displayWidth, displayHeight);
    ...
    function applySharpnessFilter(context, width, height, amount) {
        if (amount === 0 || !context || width <= 0 || height <= 0) return;
        ...
        try {
            ...
            for (let i = 0; i < data.length; i += 4) {
                ...
                data[i] = Math.min(255, Math.max(0, data[i] + normalizedAmount * (data[i] - 128)));
                data[i + 1] = Math.min(255, Math.max(0, data[i + 1] + normalizedAmount * (data[i + 1] - 128)));
                data[i + 2] = Math.min(255, Math.max(0, data[i + 2] + normalizedAmount * (data[i + 2] - 128)));
            }
            context.putImageData(imageData, 0, 0);
        } catch (error) {
            console.warn("Sharpness filter error:", error);
        }
    }

    // ── Apply Filter Presets ──
    function applyFilterPreset(context, width, height, preset) {
        if (!context || width <= 0 || height <= 0 || !preset || preset === 'none') return;
        ...
        try {
            ...
            console.warn("Filter preset error:", error);
        }
    }
    ...
    function renderCropOverlay() {
        ...
        try {
            ...
            // Validate drawing parameters
            if (!isFinite(cx) || !isFinite(cy) || !isFinite(cw) || !isFinite(ch)) return;
            ...
            console.warn("Crop overlay render error:", error);
        }
    }
    ...
    function loadImage(file) {
        ...
        img.onload = function () {
            try {
                ...
                if (!img.naturalWidth || !img.naturalHeight || !isFinite(img.naturalWidth) || !isFinite(img.naturalHeight)) {
